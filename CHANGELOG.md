...
- Fixed the Transformer ignoring a change to an attached node made between two pointer moves of a resize, for example a width applied from React state after `transform` fired: since 10.3.1 it reported the old size and the next move resized from it, so the shape jumped, and the outline was not refreshed at all, also after the gesture ended
